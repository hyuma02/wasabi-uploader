...
WEEKS = [
    {
        "period": "2026年8月31日〜9月6日",
        "no": 4,
        "summary": "経営層向けの国内最新統計（被害3.5億円・停止36日）が核。中堅の長期停止例・製造業の定量データ・クラウド大手の可用性障害が揃った週。",
        "picks": [
            ("トレンドマイクロ×CIO Lounge調査：ランサム被害は平均3.48億円・停止36日",
             "課題の裏付け・コスト正当化。経営層向け国内調査の最新値",
             "被害 平均約3.48億円（前年比+1.3億円）・停止約36日（DB 9/1）"),
            ("化粧品のフェース：暗号化＋消去で復旧は10月末見込み（約4.5か月）",
             "掴み。中堅×VPN侵入という国内最多パターンの直近例",
             "6/18侵入・暗号化＋消去・バックアップ体制を再整備へ（DB 9/1）"),
            ("Dragos：産業分野のランサム被害がQ2最多1,140件、製造業65%",
             "製造業・食品向けの定量根拠。IT系の暗号化だけで工場は止まる",
             "Q2産業被害1,140件（+12%）・製造業747件（65%）（DB 9/4）"),
            ("Microsoft 365全世界障害：証明書更新漏れで完全復旧に数日（8/31発生）",
             "BCP・単一依存リスク。M365障害中は別クラウドのバックアップが効く",
             "8/31発生・Exchange/Teams等停止・残影響は9/4まで（DB 9/5）"),
        ],
        "note": "補欠: なし（今週の登録4件を全件ピック）。ネバダ州エントリは発生時期を2025年8月に訂正済（9/5）。",
        "feature": {
            "eyebrow_jp": "国内調査", "eyebrow_en": "SURVEY: TREND MICRO x CIO LOUNGE",
            "headline": "ランサム被害企業の累計被害額は平均3.48億円、業務停止は平均36日に",
            "sub": "トレンドマイクロとCIO Loungeの国内調査（8/21公表）。被害額は前年度調査から約1.3億円増え、停止の長期化が被害を膨らませている。",
            "left_title": "調査の要点",
            "left_rows": [
                ("調査対象", ["過去3年に被害を受けた国内法人（500名以上）の", "経営者・セキュリティ責任者306人"]),
                ("被害額", ["ランサム被害企業の累計被害額は", "平均約3億4,800万円"]),
                ("業務停止", ["復旧までの平均停止時間は約862.9時間", "（約36.0日）"]),
                ("高額化", ["被害額「5億円以上」のレンジの", "割合が増加"]),
            ],
            "right_title": "平均被害額は1年で約1.6倍に",
            "right_bars": [
                ("2024年度調査", 2.2, "約2.2億円"),
                ("2026年調査", 3.48, "約3.48億円"),
            ],
            "right_note": "※ 復旧が長引くほど被害額が膨らむ（平均停止36日）。ニチレイは約10日で全面復旧。",
            "callout": ["被害額の主因は身代金ではなく「止まっている時間」。バックアップを",
                        "Object Lockで守れば、復旧の起点を失わず停止期間を短縮できる。"],
            "takeaway": "「止まっている時間」が被害額を決める。復旧できる形でバックアップを守る投資の根拠に。",
            "source": "出典：トレンドマイクロ×CIO Lounge「セキュリティ成熟度と被害の実態調査 2026」（2026年8月21日公表）より作成。",
        },
    },
    {
        ...
        "picks": [
            ("米ネバダ州：バックアップ削除で60超の州機関が停止（2025年8月発生）",
             "掴み。「攻撃者はまずバックアップを消す」の大規模実例（自治体・公共向け）",
             "2025/8/24発生・バックアップ削除→暗号化・28日で約90%復旧"),
        ...
        "feature": {
            ...
            "headline": "米ネバダ州、バックアップを削除され60超の州機関が停止（2025年8月）",
            "sub": "2025年8月の事例。攻撃者はまずバックアップを消し、それから暗号化。州は身代金を拒否し28日で約90%を復旧した。",
            "left_title": "事件の経過（2025年）",
            "left_rows": [
                ("5/14 侵入", ["偽広告経由のマルウェア入り管理ツールで侵入", "（発覚まで約3か月の潜伏）"]),
